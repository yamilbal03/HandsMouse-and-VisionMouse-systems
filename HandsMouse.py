    ...
    if len(lista) != 0:
        ...
        if dedos[1] == 1 and dedos[2] == 1:  # Si el indice esta arriba y el corazon tambien
            ...
            if longitud < 30:
                ...
                #------------------- Comprobar si esta en modo click derecho -------------------------
        if dedos[0] == 1 and dedos[4] == 1:  # Si el indice esta arriba y el corazon tambien
            # --------------->Modo click: encontrar la distancia entre ellos-------------------------
            longitud, frame, linea = detector.distancia(4,20,frame) #Nos entrega la distancia entre el punto 4 y 20
            print(longitud)
            if longitud < 90:
                cv2.circle(frame, (linea[4],linea[5]), 10, (0,255,0), cv2.FILLED)
        ...
        #----------------------------- Comprobar si esta en modo page_up -------------------------
        if dedos[0] == 1 and dedos[1] == 1:  # Si el pulgar esta arriba y el indice tambien
            longitud, frame, linea = detector.distancia(4,8,frame)
            if longitud < 40:
                #cv2.circle(frame, (linea[4],linea[5]), 10, (0,255,0), cv2.FILLED)
                #-------------------- Hacemos click si la distancia es corta ---------------------------
                subir = autopy.key.Code.PAGE_UP
                autopy.key.tap(key=subir)

        #----------------------------- Comprobar si esta en modo page_down -------------------------
        if dedos[0] == 1 and dedos[2] == 1:  # Si el pulgar esta arriba y el corazon tambien
            longitud, frame, linea = detector.distancia(4,12,frame)
            if longitud < 50:
                #cv2.circle(frame, (linea[4],linea[5]), 10, (0,255,0), cv2.FILLED)
                #-------------------- Hacemos click si la distancia es corta ---------------------------
                bajar = autopy.key.Code.PAGE_DOWN
                autopy.key.tap(key=bajar)
